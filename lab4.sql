...
select t2.supp_id, t2.supp_name, t2.rat_ratstars,
case
	when t2.rat_ratstars = 5 then "Excellent Service"
    when t2.rat_ratstars > 3 then "Good Service"
    when t2.rat_ratstars > 2 then "Average Service"
    else "Poor Service"
End
as type_of_service from
(select S.supp_id, S.supp_name, R.rat_ratstars from Supplier S, `Order`, Supplier_Pricing
inner join Rating R where R.ord_id = `order`.ord_id and `order`.pricing_id = supplier_pricing.pricing_id
and supplier_pricing.supp_id = S.supp_id) as t2;